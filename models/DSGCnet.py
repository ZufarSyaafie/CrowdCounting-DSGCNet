...
# Optional import for GCN (torch_geometric may not be available in some envs like Kaggle offline)
HAS_PYG = True
try:
...
class DSGCnet(nn.Module):
    def __init__(self, backbone, row=2, line=2, use_gcn: bool = True):
        super().__init__()
        self.backbone = backbone
        self.num_classes = 2
        num_anchor_points = row * line
        self.use_gcn = bool(use_gcn) and HAS_PYG
        ...
        if self.use_gcn:
            ...
            # Placeholders to keep state dict shape stable; not used in forward
            self.density_gcn = None
            self.feature_gcn = None
            self.alpha = nn.Parameter(
                torch.tensor([0.0, 0.0], dtype=torch.float32, requires_grad=False)
            )
...
def build(args, training):
    ...
    use_gcn = True
    if hasattr(args, "use_gcn"):
        use_gcn = bool(getattr(args, "use_gcn"))
    model = DSGCnet(backbone, args.row, args.line, use_gcn=use_gcn)
    if not training:
        return model
